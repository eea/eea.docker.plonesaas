        ...
        node(label: 'docker-host') {
          script {
            try {
              ...
              sh '''docker run -i --rm --link=${IMAGE_NAME}:plone --name=${IMAGE_NAME}-test --entrypoint /plone/instance/bin/zopepy ${IMAGE_NAME} -c "from six.moves.urllib.request import urlopen; import time; time.sleep(45); con = urlopen('http://plone:8080'); print(con.read())"'''
